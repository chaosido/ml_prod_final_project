...
def validate_audio_file(file_path: Union[str, Path], min_size_kb: int = 1, min_duration_sec: float = 0.5) -> bool:
    """
    ...
    try:
        ...
        if info.duration < min_duration_sec:
            raise ValueError(f"Duration too short: {info.duration:.2f}s (Min: {min_duration_sec}s)")
